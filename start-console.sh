...
if [ -x "$(command -v podman)" ]; then
    if [ "$(uname -s)" = "Linux" ]; then
        ...
        BRIDGE_PLUGINS="${PLUGIN_NAME}=http://localhost:9002"
    ...
    $CONSOLE_IMAGE
        ...
        BRIDGE_PLUGINS="${PLUGIN_NAME}=http://host.containers.internal:9002"
    ...
    BRIDGE_PLUGINS="${PLUGIN_NAME}=http://host.docker.internal:9002"
